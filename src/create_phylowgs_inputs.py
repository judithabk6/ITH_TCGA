...
"""
the object of that script (mostly inspired from https://github.com/morrislab/phylowgs/blob/master/parser/create_phylowgs_inputs.py 3cf19f2)
is to adapt the PhyloWGS Parser to custom CSV file formatted for PyClone
as input for the variant part.
"""
...
class PyCloneCSVParser:
  def __init__(self, vcf_filename, tumor_sample=None):
    self._vcf_filename = vcf_filename
    self._tumor_sample = tumor_sample

  def list_variants(self):
    variant_table = pd.read_csv(self._vcf_filename, sep='\t')
    variant_table = variant_table.assign(CHROM=lambda x: x.mutation_id.str.split('_').str[0].str[3:])
    variant_table = variant_table.assign(POS=lambda x: x.mutation_id.str.split('_').str[1].astype(int))
    variants_and_reads = []
    for idx, row in variant_table.iterrows():
      variants_and_reads.append((VariantClass(row['CHROM'], row['POS']), row['ref_counts'], row['ref_counts']+row['var_counts']))
    return variants_and_reads
...
class MultisampleCnvCombiner(object):
  ...
  def _get_abnormal_state_for_all_samples(self, chrom, cnv):
    ...
    if len(retained_sampidxs) != len(set(retained_sampidxs)):
...
def parse_variants(samples, vcf_files, vcf_types, tumor_sample, missing_variant_confidence):
  ...
  for sample in samples:
    ...
    elif vcf_type == 'csv_pyclone':
      variant_parser = PyCloneCSVParser(vcf_fn, tumor_sample)
    else:
