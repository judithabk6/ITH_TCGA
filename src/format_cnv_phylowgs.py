...
cnv_table = pd.read_csv('results/{}/pyclone/ascat_hg38_cnv_table.csv'.format(patient), sep='\t')
with open('results/{}/pyclone/ascat_purity.txt'.format(patient), 'r') as f:
    purity = float(f.read())
...
cnv_table[output_fields].to_csv('results/{}/PhyloWGS/cnv_table.csv'.format(patient), sep='\t')
